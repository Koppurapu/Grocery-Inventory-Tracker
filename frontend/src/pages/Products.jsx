...
function Products() {
  ...
  const [formData, setFormData] = useState({ name: '', sku: '', quantity: '', price: '', cost: '', location: '', category: '', supplier: '', reorderLevel: '50' });
  ...
  const fetchItems = async () => {
    try {
      ...
      setItems(await apiFetch(`/items?${params}`));
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const body = { ...formData, quantity: parseInt(formData.quantity), price: parseFloat(formData.price), cost: parseFloat(formData.cost || 0), reorderLevel: parseInt(formData.reorderLevel || 50) };
      if (editingItem) {
  ...
  const openEdit = (item) => {
    setEditingItem(item);
    setFormData({ name: item.name, sku: item.sku, quantity: item.quantity, price: item.price, cost: item.cost, location: item.location, category: item.category, supplier: item.supplier, reorderLevel: item.reorderLevel });
    setShowModal(true);
  };

  const closeModal = () => { setShowModal(false); setEditingItem(null); setFormData({ name: '', sku: '', quantity: '', price: '', cost: '', location: '', category: '', supplier: '', reorderLevel: '50' }); };
  ...
  return (
    <div className="page-container">
      <div className="page-header">
        ...
        <button className="btn-primary" onClick={() => setShowModal(true)}><Plus size={18} /> Add Product</button>
      </div>
      ...
      <div className="data-table-container">
        <table className="data-table">
          <thead>
            <tr><th>Product</th><th>SKU</th><th>Category</th><th>Quantity</th><th>Price</th><th>Location</th><th>Status</th><th>Actions</th></tr>
          </thead>
          <tbody>
            {items.map(item => (
              <tr key={item.id}>
                ...
                <td className="actions-cell">
                  <button className="action-btn" title="Edit" onClick={() => openEdit(item)}><Edit size={16} /></button>
                  <button className="action-btn danger" title="Delete" onClick={() => handleDelete(item.id)}><Trash2 size={16} /></button>
                </td>
              </tr>
            ))}
          </tbody>
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-row">
                ...
                <div className="form-group"><label>Reorder Level</label><input type="number" value={formData.reorderLevel} onChange={(e) => setFormData({...formData, reorderLevel: e.target.value})} /></div>
              </div>
